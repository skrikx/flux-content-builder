...
-- Policies: owner only
create policy if not exists brands_owner_select on public.brands
  for select using (auth.uid() = user_id);
create policy if not exists brands_owner_all on public.brands
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

create policy if not exists content_owner_select on public.content
  for select using (auth.uid() = user_id);
create policy if not exists content_owner_all on public.content
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

create policy if not exists schedules_owner_select on public.schedules
  for select using (auth.uid() = user_id);
create policy if not exists schedules_owner_all on public.schedules
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

create policy if not exists prefs_owner_select on public.provider_prefs
  for select using (auth.uid() = user_id);
create policy if not exists prefs_owner_all on public.provider_prefs
  for all using (auth.uid() = user_id) with check (auth.uid() = user_id);

-- Storage policies for content-assets bucket
create policy if not exists "Users can view content assets" 
on storage.objects 
for select 
using (bucket_id = 'content-assets');

create policy if not exists "Users can upload content assets" 
on storage.objects 
for insert 
with check (bucket_id = 'content-assets' AND auth.uid()::text = (storage.foldername(name))[1]);

create policy if not exists "Users can update their content assets" 
on storage.objects 
for update 
