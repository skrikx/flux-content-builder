...
function normalizeIdeas(query: string, docs: SearchResult[]): Idea[] {
...
serve(async (req) => {
  ...
  try {
    ...
    if (sources.includes('tavily') && tavKey) {
      ...
      }).then(r=>r.json()).then((j: TavilyResponse)=> (j?.results || []).map((r: TavilyResult)=>({ title:r.title, snippet:r.content, url:r.url, source:'tavily'}))).catch(()=>[]));
    }
    ...
    if (sources.includes('reddit')) {
      tasks.push(fetch(`https://www.reddit.com/search.json?q=${encodeURIComponent(query)}&sort=top&t=year&limit=10`, { headers: { 'User-Agent':'flux-content/1.0' } })
        .then(r=>r.json()).then((j: RedditResponse)=> (j.data?.children || []).map((c: RedditChild)=>({ title:c.data?.title, snippet:c.data?.selftext||c.data?.title, url:`https://reddit.com${c.data?.permalink}`, source:'reddit'}))).catch(()=>[]));
    }
    ...
    if (sources.includes('hn')) {
      tasks.push(fetch(`https://hn.algolia.com/api/v1/search?query=${encodeURIComponent(query)}&tags=story`)
        .then(r=>r.json()).then((j: HNResponse)=> (j.hits || []).map((h: HNResult)=>({ title:h.title, snippet:h._highlightResult?.title?.value || h.title, url:h.url||`https://news.ycombinator.com/item?id=${h.objectID}`, source:'hn'}))).catch(()=>[]));
    }
    ...
    if (sources.includes('wikipedia')) {
      tasks.push(fetch(`https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(query)}&format=json&origin=*`)
        .then(r=>r.json()).then((j: WikiResponse)=> (j.query?.search || []).map((s: WikiResult)=>({ title:s.title, snippet:s.snippet?.replace(/<[^>]+>/g,''), url:`https://en.wikipedia.org/wiki/${encodeURIComponent(s.title)}`, source:'wikipedia'}))).catch(()=>[]));
    }
